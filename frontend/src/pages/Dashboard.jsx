...
const Dashboard = () => {
  ...
  const [consoleLogs, setConsoleLogs] = useState([
    {
      ...
      text: "Central System Ready. Select a classroom to monitor.",
      type: "system",
    },
  ]);
    ...
    const fetchRooms = async () => {
      try {
        ...
        if (roomsData.length > 0) {
          ...
          addLog(
            `Loaded ${roomsData.length} classrooms successfully.`,
            "system",
          );
        } else {
  ...
  useEffect(() => {
    ...
    addLog(
      `Switched monitoring to classroom ID: ${selectedRoomId}.`,
      "success",
    );
  ...
  const fetchDashboardData = async (roomId, isPoll = false) => {
    try {
      ...
      setTelemetry({
        temperature:
          telemetryData.temperature !== null
            ? parseFloat(telemetryData.temperature).toFixed(1)
            : "--",
        humidity:
          telemetryData.humidity !== null
            ? parseFloat(telemetryData.humidity).toFixed(1)
            : "--",
        motion: !!telemetryData.motion,
        motion_status: telemetryData.motion
          ? "Active movement detected"
          : "Room inactive / Empty space",
      ...
      if (!isPoll || historyLogs.length === 0) {
        const historyData = await telemetryService.getHistory(
          roomId,
          historyHours,
        );
  ...
  const handleDeviceToggle = async (deviceId, deviceName) => {
    ...
    try {
      ...
      setDevices((prev) =>
        prev.map((d) =>
          d.id === deviceId ? { ...d, status: updatedDevice.status } : d,
        ),
      );
      addLog(
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="flex items-center gap-2 bg-[#091124]/40 border border-blue-950 p-2 rounded-lg backdrop-blur-sm">
            <label className="text-xs font-mono text-blue-400 font-semibold">
              CLASSROOM:
            </label>
            <select
          ...
          <div className="flex items-center gap-2 bg-[#091124]/40 border border-blue-950 p-2 rounded-lg backdrop-blur-sm">
            ...
            <select
              ...
              className="bg-transparent text-sm text-slate-100 focus:outline-none pr-4 cursor-pointer"
            >
              <option value="1" className="bg-[#091124]">
                Last Hour
              </option>
              <option value="6" className="bg-[#091124]">
                Last 6 Hours
              </option>
              <option value="24" className="bg-[#091124]">
                Last 24 Hours
              </option>
              <option value="168" className="bg-[#091124]">
                Last 7 Days
              </option>
            </select>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-amber-500/10 to-orange-500/10 border border-amber-500/20 p-6 shadow-md tick-corners">
          ...
          <div className="flex items-baseline">
            <span className="text-4xl font-bold text-slate-100 font-display">
              {telemetry.temperature}
            </span>
            <span className="text-xl font-medium text-slate-400 ml-1">°C</span>
          </div>
          <p className="text-[11px] text-slate-400 mt-3 font-mono">
            DHT22 environmental sensor node
          </p>
        </div>
        ...
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-blue-500/10 to-indigo-500/10 border border-blue-500/20 p-6 shadow-md tick-corners">
          ...
          <p className="text-[11px] text-slate-400 mt-3 font-mono">
            Relative atmospheric moisture index
          </p>
        </div>

        {/* Motion Card */}
        <div
          className={`relative overflow-hidden rounded-2xl border p-6 shadow-md transition-all duration-500 tick-corners ${
            telemetry.motion
        ...
        >
          ...
          <div className="flex justify-between items-center mb-2">
            ...
            <span className="flex h-2 w-2 relative">
              <span
                className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${telemetry.motion ? "bg-rose-400" : "bg-slate-400"}`}
              ></span>
              <span
                className={`relative inline-flex rounded-full h-2 w-2 ${telemetry.motion ? "bg-rose-500" : "bg-slate-500"}`}
              ></span>
            </span>
          </div>
              ...
              devices.map((device) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className={`p-2 rounded-lg ${isChecked ? "bg-indigo-500/10 text-indigo-400" : "bg-blue-950/20 text-slate-500"}`}
                      >
      ...
      <div className="bg-[#091124]/40 border border-blue-950 rounded-2xl p-6 backdrop-blur-md">
              ...
              {historyLogs.length === 0 ? (
                ...
                [...historyLogs]
                  ...
                  .map((log, index) => (
                    <tr
                      key={index}
                      className="hover:bg-blue-950/20 transition duration-100"
                    >
                      ...
                      <td className="px-6 py-3.5">
                        ...
                        >
                          <span
                            className={`h-1.5 w-1.5 rounded-full ${log.motion ? "bg-rose-500" : "bg-slate-500"}`}
                          ></span>
        ...
        {historyLogs.length > 10 && (
          <div className="mt-3 text-right text-[10px] text-slate-500">
            Showing latest 10 records. Total logged in range:{" "}
            {historyLogs.length}
          </div>
        )}
      </div>
