...
<html lang="<?php echo e(str_replace('_', '-', app()->getLocale())); ?>" class="h-full">
    <head>
        ...
        <title><?php echo e(config('app.name', 'Smart Classroom')); ?></title>
...
<?php endif; ?>
            ...
            <?php if(Route::has('login')): ?>
                <nav class="flex items-center gap-4">
                    <?php if(auth()->guard()->check()): ?>
                        <a href="<?php echo e(url('/dashboard')); ?>" class="px-5 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white font-semibold text-sm transition-all duration-200 shadow-md shadow-indigo-600/10 hover:shadow-indigo-600/20 active:scale-[0.98]">
                            Go to Dashboard
                        </a>
                    <?php else: ?>
                        <a href="<?php echo e(route('login')); ?>" class="text-sm font-semibold text-slate-300 hover:text-white transition-colors py-2 px-4">
                            Log in
                        </a>

                        <?php if(Route::has('register')): ?>
                            <a href="<?php echo e(route('register')); ?>" class="px-5 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-white border border-slate-700/60 hover:border-slate-600 font-semibold text-sm transition-all duration-200 active:scale-[0.98]">
                                Register
                            </a>
                        <?php endif; ?>
                    <?php endif; ?>
        ...
        <main class="w-full max-w-7xl mx-auto px-6 py-12 md:py-20 relative z-10 my-auto">
            <div class="max-w-4xl mx-auto text-center flex flex-col items-center justify-center space-y-6 py-8 md:py-14">
                ...
                <div class="flex flex-wrap items-center justify-center gap-4 pt-4">
                    <?php if(auth()->guard()->check()): ?>
                        <a href="<?php echo e(url('/dashboard')); ?>" class="px-6 py-3.5 rounded-xl bg-gradient-to-r from-indigo-600 to-cyan-600 hover:from-indigo-500 hover:to-cyan-500 text-white font-bold text-sm shadow-xl shadow-indigo-600/10 hover:shadow-indigo-600/20 active:scale-[0.99] transition-all duration-200">
                            Open Hub Dashboard
                        </a>
                    <?php else: ?>
                        <a href="<?php echo e(route('login')); ?>" class="px-6 py-3.5 rounded-xl bg-gradient-to-r from-indigo-600 to-cyan-600 hover:from-indigo-500 hover:to-cyan-500 text-white font-bold text-sm shadow-xl shadow-indigo-600/10 hover:shadow-indigo-600/20 active:scale-[0.99] transition-all duration-200">
                            Access Dashboard Portal
                        </a>
                        <a href="<?php echo e(route('register')); ?>" class="px-6 py-3.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-white border border-slate-700/60 font-semibold text-sm transition-all duration-200 active:scale-[0.99]">
        ...
        <footer class="w-full max-w-7xl mx-auto px-6 py-6 border-t border-slate-800/60 flex items-center justify-between text-xs text-slate-500 relative z-10">
            <p>&copy; <?php echo e(date('Y')); ?> Smart Classroom Hub. All rights reserved.</p>
...
<?php /**PATH E:\MyCODE\RUPP Lesson and Assignment\Year2\Second Semester\Computer Architecture\IOT Project\files\smart-classroom-project\smart-classroom\laravel-app\resources\views/welcome.blade.php ENDPATH**/ ?>